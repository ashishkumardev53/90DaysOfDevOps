...
if [ -z "$LOG_DIR" ] || [ ! -d "$LOG_DIR" ]; then echo "Usage: $0 <log_directory>" exit 1 fi
...
------------------------------------------------------------
echo "Compressed files: $compressed" echo "Deleted files: $deleted"
...
------------------------------------------------------------
if [ -z "$SOURCE" ] || [ -z "$DEST" ]; then echo "Usage: $0 <source_directory> <backup_destination>" exit 1 fi

if [ ! -d "$SOURCE" ]; then echo "Source directory does not exist" exit 1 fi
...
------------------------------------------------------------
TIMESTAMP=$(date +%Y-%m-%d) ARCHIVE="backup-$TIMESTAMP.tar.gz"
...
------------------------------------------------------------
if [ ! -f "$DEST/$ARCHIVE" ]; then echo "Backup failed" exit 1 fi
...
echo "Backup created: $ARCHIVE" echo "Size: $SIZE"
...
LOG_DIR="/var/log/myapp" SOURCE="/data" DEST="/backups"
...
log "Logs compressed: $compressed" log "Old archives deleted: $deleted" }
...
if [ ! -f "$DEST/$ARCHIVE" ]; then log "Backup failed" exit 1 fi

SIZE=$(du -h "$DEST/$ARCHIVE" | cut -f1)

log "Backup created: $ARCHIVE" log "Backup size: $SIZE"
